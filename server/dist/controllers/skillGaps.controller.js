...
exports.getRecommendations = exports.submitAssessment = exports.getSkillGaps = void 0;
...
const computeSkillLevels = async (userId) => {
    // 1. Get quiz performance by topic
    const attempts = await Quiz_1.QuizAttempt.find({ userId });
    ...
    const learningPaths = await LearningPath_1.default.find({ userId, isActive: true });
    ...
    const skills = TRACKED_TOPICS.map(topic => {
        ...
        if (hasData) {
            ...
            currentLevel = Math.round(weightedComponents.reduce((sum, c) => sum + (c.value * c.weight), 0) / totalWeight);
        }
...
const detectTopicFromTitle = (title) => {
    const lower = title.toLowerCase();
    if (lower.includes('javascript') || lower.includes('es6') || lower.includes('async'))
        return 'JavaScript';
    if (lower.includes('react') || lower.includes('jsx') || lower.includes('hooks') || lower.includes('component'))
        return 'React';
    if (lower.includes('typescript') || lower.includes('types') || lower.includes('generics'))
        return 'TypeScript';
    if (lower.includes('node') || lower.includes('express') || lower.includes('backend'))
        return 'Node.js';
    if (lower.includes('css') || lower.includes('flexbox') || lower.includes('grid') || lower.includes('responsive'))
        return 'CSS';
    if (lower.includes('api') || lower.includes('rest') || lower.includes('http'))
        return 'APIs';
    if (lower.includes('mongo') || lower.includes('database') || lower.includes('mongoose'))
        return 'MongoDB';
...
const getSkillGaps = async (req, res, next) => {
    try {
        ...
        if (!hasAnyData) {
            ...
            // Upsert into DB
            await SkillGap_1.default.findOneAndUpdate({ userId }, { skills: defaultSkills, lastAssessmentDate: new Date() }, { new: true, upsert: true });
            res.status(200).json({
                success: true,
                data: {
        ...
        const skillGap = await SkillGap_1.default.findOneAndUpdate({ userId }, { skills, lastAssessmentDate: new Date() }, { new: true, upsert: true });
        res.status(200).json({
            success: true,
            data: {
                ...skillGap?.toObject(),
...
const submitAssessment = async (req, res, next) => {
    try {
        ...
        const skillGap = await SkillGap_1.default.findOneAndUpdate({ userId: req.user?._id }, {
...
const getRecommendations = async (req, res, next) => {
    try {
        ...
        const recommendations = prioritySkills.map(skill => {
            ...
            const matchingPaths = learningPathTemplates_1.allLearningPaths.filter(path => path.modules.some(m => m.milestones.some(ms => ms.checklistItems.some(ci => ci.title.toLowerCase().includes(skill.topic.toLowerCase()) ||
                ...
                for (const mod of path.modules) {
                    for (const ms of mod.milestones) {
                        for (const ci of ms.checklistItems) {
                            if (ci.url && (ci.title.toLowerCase().includes(skill.topic.toLowerCase()) ||
                                (ms.quizTopic || '').toLowerCase() === skill.topic.toLowerCase())) {
                                resources.push({
                                    title: ci.title,
                                    url: ci.url,
...
//# sourceMappingURL=skillGaps.controller.js.map