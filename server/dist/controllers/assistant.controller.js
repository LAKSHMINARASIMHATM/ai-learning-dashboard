...
exports.getSuggestions = exports.clearHistory = exports.getChatHistory = exports.sendMessage = void 0;
...
const callHuggingFace = async (userMessage, conversationHistory) => {
...
const sendMessage = async (req, res, next) => {
    try {
        ...
        // Save user message
        const userMessage = await ChatMessage_1.default.create({
            userId: req.user?._id,
            type: 'user',
            content,
        });
        ...
        try {
            ...
            const recentMessages = await ChatMessage_1.default.find({ userId: req.user?._id })
                .sort({ createdAt: -1 })
                .limit(6);
        ...
        // Save AI response
        const aiMessage = await ChatMessage_1.default.create({
            userId: req.user?._id,
            type: 'ai',
            content: aiContent,
        });
...
const getChatHistory = async (req, res, next) => {
    try {
        const messages = await ChatMessage_1.default.find({ userId: req.user?._id })
            .sort({ createdAt: 1 })
            .limit(50);
...
const clearHistory = async (req, res, next) => {
    try {
        await ChatMessage_1.default.deleteMany({ userId: req.user?._id });
        res.status(200).json({
            success: true,
...
// @access  Private
const getSuggestions = async (_req, res, next) => {
    try {
        const suggestions = [
