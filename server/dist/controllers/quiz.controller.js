...
exports.getQuizRecommendations = exports.getQuizAnalytics = exports.getQuizHistory = exports.submitQuizAttempt = exports.getQuiz = exports.getQuizzes = exports.reseedQuizzes = void 0;
...
const buildQuizzesFromBank = () => {
    ...
    const selectQuestions = (questions, count) => shuffleArray(questions).slice(0, count);
    return [
        {
            ...
            timeLimit: 15,
            passingScore: 70,
            questions: selectQuestions(questionBank_1.javascriptQuestions.filter(q => q.difficulty === 'beginner'), 8),
        },
        {
            ...
            timeLimit: 20,
            passingScore: 70,
            questions: selectQuestions(questionBank_1.javascriptQuestions.filter(q => q.difficulty === 'intermediate'), 10),
        },
        {
            ...
            timeLimit: 25,
            passingScore: 75,
            questions: selectQuestions(questionBank_1.javascriptQuestions.filter(q => q.difficulty === 'advanced'), 8),
        },
        {
            ...
            timeLimit: 15,
            passingScore: 70,
            questions: selectQuestions(questionBank_1.reactQuestions.filter(q => q.difficulty === 'beginner'), 8),
        },
        {
            ...
            timeLimit: 20,
            passingScore: 70,
            questions: selectQuestions(questionBank_1.reactQuestions.filter(q => q.difficulty === 'intermediate'), 10),
        },
        {
            ...
            timeLimit: 25,
            passingScore: 75,
            questions: selectQuestions(questionBank_1.reactQuestions.filter(q => q.difficulty === 'advanced'), 8),
        },
        {
            ...
            timeLimit: 15,
            passingScore: 70,
            questions: selectQuestions(questionBank_1.typescriptQuestions.filter(q => q.difficulty === 'beginner'), 6),
        },
        {
            ...
            timeLimit: 20,
            passingScore: 70,
            questions: selectQuestions(questionBank_1.typescriptQuestions.filter(q => q.difficulty === 'intermediate' || q.difficulty === 'advanced'), 10),
        },
        {
            title: 'Node.js & APIs',
            ...
            timeLimit: 20,
            passingScore: 70,
            questions: questionBank_1.nodejsQuestions,
        },
        {
            ...
            timeLimit: 15,
            passingScore: 70,
            questions: questionBank_1.cssQuestions,
        },
        {
            ...
            passingScore: 70,
            questions: [
                ...selectQuestions(questionBank_1.javascriptQuestions, 5),
                ...selectQuestions(questionBank_1.reactQuestions, 5),
                ...selectQuestions(questionBank_1.typescriptQuestions, 3),
                ...selectQuestions(questionBank_1.nodejsQuestions, 2),
            ],
        },
        {
            ...
            timeLimit: 10,
            passingScore: 50,
            questions: selectQuestions([...questionBank_1.javascriptQuestions, ...questionBank_1.reactQuestions, ...questionBank_1.cssQuestions], 5),
        },
        {
            ...
            passingScore: 65,
            questions: [
                ...selectQuestions(questionBank_1.cssQuestions, 4),
                ...selectQuestions(questionBank_1.javascriptQuestions.filter(q => q.difficulty === 'beginner'), 6),
            ],
        },
        {
            ...
            timeLimit: 25,
            passingScore: 75,
            questions: selectQuestions(questionBank_1.javascriptQuestions.filter(q => q.difficulty === 'intermediate' || q.difficulty === 'advanced'), 12),
        },
        {
        ...
        },
        {
            ...
            timeLimit: 25,
            passingScore: 80,
            questions: selectQuestions(questionBank_1.typescriptQuestions, 10),
        },
        {
...
const generateRecommendations = (topicPerformance, _errorPatterns) => {
...
exports.reseedQuizzes = reseedQuizzes;
const getResourcesForTopic = (topic, level) => {
    const resources = {
        JavaScript: {
            beginner: [
...
const getQuizzes = async (req, res, next) => {
    try {
        ...
        const count = await Quiz_1.Quiz.countDocuments();
        if (count === 0) {
            const quizzes = buildQuizzesFromBank();
            await Quiz_1.Quiz.insertMany(quizzes);
        }
        ...
        const quizzes = await Quiz_1.Quiz.find(query).select('-questions.correctAnswer');
        res.status(200).json({
            success: true,
            count: quizzes.length,
            data: quizzes,
...
const getQuiz = async (req, res, next) => {
    try {
        const quiz = await Quiz_1.Quiz.findById(req.params.id).select('-questions.correctAnswer -questions.explanation');
        if (!quiz) {
            res.status(404).json({
                success: false,
...
const submitQuizAttempt = async (req, res, next) => {
    try {
        ...
        const attempt = await Quiz_1.QuizAttempt.create({
            userId: req.user?._id,
            quizId,
        ...
        const topicPerformance = Object.fromEntries(Object.entries(topicScores).map(([topic, { correct, total }]) => [
            topic,
        ...
        const reviewData = quiz.questions.map(q => ({
            ...
            userAnswer: processedAnswers.find((a) => a.questionId === q.questionId)?.selectedAnswer,
            isCorrect: processedAnswers.find((a) => a.questionId === q.questionId)?.isCorrect,
        }));
        res.status(201).json({
            success: true,
            data: {
...
const getQuizHistory = async (req, res, next) => {
    try {
        const attempts = await Quiz_1.QuizAttempt.find({ userId: req.user?._id })
...
const getQuizAnalytics = async (req, res, next) => {
    try {
        ...
        const quizzes = await Quiz_1.Quiz.find({ _id: { $in: quizIds } });
...
exports.getQuizRecommendations = getQuizRecommendations;
